...
class API():
    ...
    def get_signals(self, page: int = 0, limit: int = 10) -> dict:
        request = requests.get(self.base_url + f'/signals?page={page}&limit={limit}')
        return request.json()
    
    def get_candidates(self, page: int = 0, limit: int = 10) -> dict:
        request = requests.get(self.base_url + f'/candidates?page={page}&limit={limit}')
        return request.json()
    
    def update_candidates(self, content_id: str, scores: dict):
        requests.post(self.base_url + '/candidates/update', json={
            'content_id': content_id,
            'scores': scores
        })