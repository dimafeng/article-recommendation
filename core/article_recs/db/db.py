...
class Database(object):
    ...
    def update_content(self, content_id: str, data: dict):
        with self.Session() as session:
            ...
            session.add(Event(type="content_updated", data={"content_id": content.id, "fields": list(data.keys())}))
            session.commit()
    ...
    def add_content(self, content: Content):
        with self.Session() as session:
            ...
            if(existing_content is None):
                ...
                session.add(Event(type="content_added", data={"content_id": content.id}))
                session.add(content)
                session.commit()
            else:
                logging.info("Content %s already exists. Updating...", content.id)
    ...
    def get_contents(self, content_ids: list):
        with self.Session() as session:
    ...
    def read_latest_events(self, consumer_id: str, event_type: str, limit: int, save_checkpoint: bool = True):
        ...
        with self.Session() as session:
            ...
            if(len(event_consumers) == 0):
                session.add(EventConsumer(id=consumer_id, event_type=event_type, checkpoint=0))
                event_consumers = session.query(EventConsumer).filter(EventConsumer.id == consumer_id).all()

            event_consumer = event_consumers[0]
            events = session.query(Event).filter(Event.type == event_type).filter(Event.id > event_consumer.checkpoint).limit(limit).all()
    ...
    def delete_canidates_older_than(self, days: int):
        with self.Session() as session:
            candidates = session.query(Candidate).filter(Candidate.created < datetime.now() - timedelta(days=days)).limit(1000).all()
    ...
    def delete_oldest_candidates(self, limit: int):
        with self.Session() as session:
            candidates = session.query(Candidate).order_by(Candidate.created).limit(limit).all()
            for candidate in candidates:
                session.delete(candidate)
            session.commit()
